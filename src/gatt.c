...
static GChecksum *checksum(GList *database)
{
	...
	for (list = database; list; list = g_list_next(list)) {
		struct btd_attribute *attr = list->data;
		uint8_t buf[ATT_DEFAULT_LE_MTU];
		size_t bufsize, size;

		/* Computing checksum for handle */
		bufsize = size = sizeof(attr->handle);
		memcpy(buf, &attr->handle, size);
		g_checksum_update(chksum, buf, size);

		/* Computing checksum for UUID/Type */
		size = sizeof(attr->type);
		memcpy(&buf[bufsize], &attr->type, size);
		bufsize+= size;
		g_checksum_update(chksum, buf, size);
		...
		if ((bt_uuid_cmp(&primary_uuid, &attr->type) == 0) ||
			...
			size = attr->value_len;
			memcpy(&buf[bufsize], &attr->value, size);
			bufsize+= size;
			g_checksum_update(chksum, buf, size);
		}
	}
